...
public class DbIntrusionRepo {
	...
	public long insert(Intrusion intru){
		...
		try {
			statement = connection.prepareStatement("insert into Intrusion(severity, timestamp) values(?, ?);", PreparedStatement.RETURN_GENERATED_KEYS);
	...
	public List<Intrusion> getAll(){
		...
		try {
			...
			while(resultSet.next())
				intrus.add(new Intrusion(resultSet.getInt(1), resultSet.getInt(2), resultSet.getDate(3)));
	...
	public Intrusion get(long id){
		...
		try {
			...
			if(resultSet.next())
				return new Intrusion(resultSet.getLong(1), (int)resultSet.getLong(2), resultSet.getDate(3));
